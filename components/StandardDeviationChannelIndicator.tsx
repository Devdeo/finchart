...
interface StandardDeviationChannelIndicatorProps {
  chart: any;
  onRemove: () => void;
...
const StandardDeviationChannelIndicator: React.FC<StandardDeviationChannelIndicatorProps> = ({
  chart,
  onRemove,
  initialPeriod = 20,
  initialMultiplier = 2
  ...
  const registerStdDevChannel = (klinecharts: any) => {
    try {
      klinecharts.registerIndicator({
        name: "MY_STDDEV_CHANNEL",
        shortName: "SDC",
        series: "price",
        calcParams: [period, multiplier],
        figures: [
          { key: "mean", title: "Mean", type: "line" },
        ...
        calc: (dataList: any[], indicator: any) => {
          ...
          const m = Number(indicator.calcParams[1]) || 2;
          const result: any[] = [];
          ...
          for (let i = 0; i < dataList.length; i++) {
            ...
            const window = dataList.slice(i - p + 1, i + 1);
        ...
        styles: {
          lines: [
            { key: "mean", color: colors.mean, size: thickness, style: "solid" },
            { key: "upper", color: colors.upper, size: 1, style: "dashed" },
            { key: "lower", color: colors.lower, size: 1, style: "dashed" },
          ],
        },
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const initStdDevChannel = async () => {
      const klinecharts = await import("klinecharts");
      registerStdDevChannel(klinecharts);
      const id = chart.createIndicator("MY_STDDEV_CHANNEL", true, { id: "candle_pane" });
  ...
  useEffect(() => {
    ...
    chart.overrideIndicator({
      ...
      styles: { 
        lines: [
          { key: "mean", color: colors.mean, size: thickness },
          { key: "upper", color: colors.upper, size: 1 },
          { key: "lower", color: colors.lower, size: 1 }
        ] 
      },
    });
  ...
  const handleColorChange = (type: 'mean' | 'upper' | 'lower', color: string) => {
        ...
        >
          <div style={{ 
            ...
            <span style={{
              ...
              Standard Deviation Channel Settings
            </span>
            <button
              style={{
                background: "none",
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <input
                type="color"
                value={colors.mean}
                onChange={(e) => handleColorChange('mean', e.target.value)}
                onMouseDown={(e) => e.stopPropagation()}
                onClick={(e) => e.stopPropagation()}
                style={{
                  width: "40px",
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: "11px", color: "#787b86" }}>Upper</label>
                <input
                  type="color"
                  value={colors.upper}
              ...
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: "11px", color: "#787b86" }}>Lower</label>
                <input
                  type="color"
                  value={colors.lower}
...
export default StandardDeviationChannelIndicator;
