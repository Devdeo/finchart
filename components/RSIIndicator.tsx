...
const RSIIndicator: React.FC<RSIIndicatorProps> = ({
  chart,
  onRemove,
  initialPeriod = 14,
  initialColor = "#FF9800"
  ...
  const registerRSI = (klinecharts: any) => {
    try {
  ...
  useEffect(() => {
    ...
    const initRSI = async () => {
      ...
      registerRSI(klinecharts);
      const id = chart.createIndicator("MY_RSI", false);
  ...
  useEffect(() => {
    ...
    chart.overrideIndicator({
      id: indicatorIdRef.current,
      styles: { lines: [{ key: "rsi", color, size: thickness }] },
    });
