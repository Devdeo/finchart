...
const EMAIndicator: React.FC<EMAIndicatorProps> = ({
  chart,
  onRemove,
  initialPeriod = 20,
  initialColor = "#2196F3"
  ...
  const registerEMA = (klinecharts: any) => {
    try {
  ...
  useEffect(() => {
    ...
    const initEMA = async () => {
      ...
      registerEMA(klinecharts);
      const id = chart.createIndicator("MY_EMA", true, { id: "candle_pane" });
  ...
  useEffect(() => {
    ...
    chart.overrideIndicator({
      id: indicatorIdRef.current,
      styles: { lines: [{ key: "ema", color, size: thickness }] },
    });
