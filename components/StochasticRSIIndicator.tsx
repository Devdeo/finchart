...
interface StochasticRSIIndicatorProps {
  chart: any;
  onRemove: () => void;
  initialRsiPeriod?: number;
  initialStochPeriod?: number;
...
const StochasticRSIIndicator: React.FC<StochasticRSIIndicatorProps> = ({
  chart,
  onRemove,
  initialRsiPeriod = 14,
  ...
  const registerStochasticRSI = (klinecharts: any) => {
    try {
      klinecharts.registerIndicator({
        name: "MY_STOCHRSI",
        shortName: "StochRSI",
        series: "indicator",
        calcParams: [rsiPeriod, stochPeriod, kPeriod, dPeriod],
        figures: [
          {
            key: "k",
            title: "%K",
        ...
        calc: (dataList: any[], indicator: any) => {
          const [rsiP, stochP, kP, dP] = indicator.calcParams;
          if (rsiP <= 0 || stochP <= 0 || kP <= 0 || dP <= 0 || rsiP > dataList.length) return [];
          ...
          for (let i = 0; i < dataList.length; i++) {
            if (i === 0) {
              rsiValues.push(50); // neutral starting point
              continue;
            }
            ...
            if (i < rsiP) {
              ...
              const avgGain = gains.reduce((a, b) => a + b, 0) / rsiP;
              const avgLoss = losses.reduce((a, b) => a + b, 0) / rsiP;
          ...
          for (let i = 0; i < rsiValues.length; i++) {
            ...
            const rsiSlice = rsiValues.slice(i - stochP + 1, i + 1);
            const highestRSI = Math.max(...rsiSlice);
            const lowestRSI = Math.min(...rsiSlice);

            const stochRSI = highestRSI === lowestRSI ? 50 :
          ...
          for (let i = 0; i < stochRSIValues.length; i++) {
            ...
            const kSlice = stochRSIValues.slice(i - kP + 1, i + 1);
            const k = kSlice.reduce((a, b) => a + b, 0) / kSlice.length;
            kValues.push(k);
          }
          ...
          for (let i = 0; i < kValues.length; i++) {
            ...
            if (k !== null && i >= dP - 1) {
              const dSlice = kValues.slice(i - dP + 1, i + 1).filter(v => v !== null);
              if (dSlice.length === dP) {
  ...
  useEffect(() => {
    ...
    const initStochasticRSI = async () => {
      const klinecharts = await import("klinecharts");
      registerStochasticRSI(klinecharts);
      const id = chart.createIndicator("MY_STOCHRSI", false);
  ...
  }, [rsiPeriod, stochPeriod, kPeriod, dPeriod, chart]);
  ...
  const handleRsiPeriodChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleStochPeriodChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseInt(e.target.value);
    if (value >= 1 && value <= 200) {
      setStochPeriod(value);
    }
  };
  ...
  return (
    <div style={{ position: "relative", display: "inline-block", margin: "2px" }} ref={settingsRef}>
      ...
      >
        ...
        >
          StochRSI({rsiPeriod},{stochPeriod},{kPeriod},{dPeriod})
        </span>
        ...
        >
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <input
              ...
              max={200}
              value={stochPeriod}
              onChange={handleStochPeriodChange}
              onMouseDown={(e) => e.stopPropagation()}
              onClick={(e) => e.stopPropagation()}
              style={{
                width: "100%",
...
export default StochasticRSIIndicator;
