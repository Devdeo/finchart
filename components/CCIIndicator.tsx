...
const CCIIndicator: React.FC<CCIIndicatorProps> = ({
  chart,
  onRemove,
  initialPeriod = 20,
  initialColor = "#2196F3"
  ...
  const registerCCI = (klinecharts: any) => {
    try {
      klinecharts.registerIndicator({
        ...
        calc: (dataList: any[], indicator: any) => {
          ...
          for (let i = 0; i < dataList.length; i++) {
            ...
            const sma = tps.reduce((a, b) => a + b, 0) / p;
            const meanDev = tps.reduce((a, b) => a + Math.abs(b - sma), 0) / p;

            const cci = meanDev !== 0 ? (tp - sma) / (0.015 * meanDev) : 0;
            result.push({ cci });
          }
          return result;
        },
  ...
  useEffect(() => {
    ...
    const initCCI = async () => {
      ...
      registerCCI(klinecharts);
      const id = chart.createIndicator("MY_CCI", false);
  ...
  useEffect(() => {
    ...
    chart.overrideIndicator({
      id: indicatorIdRef.current,
      styles: { lines: [{ key: "cci", color, size: thickness }] },
    });
