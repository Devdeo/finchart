...
interface KeltnerChannelIndicatorProps {
  chart: any;
  onRemove: () => void;
...
const KeltnerChannelIndicator: React.FC<KeltnerChannelIndicatorProps> = ({
  chart,
  onRemove,
  initialPeriod = 20,
  initialMultiplier = 2
  ...
  const registerKC = (klinecharts: any) => {
    ...
    try {
      klinecharts.registerIndicator({
        ...
        calc: (dataList: any[], indicator: any) => {
          ...
          // EMA of closes
          const midArr = ema(closes, p);

          // True Range
          const tr: number[] = [];
          for (let i = 0; i < dataList.length; i++) {
            if (i === 0) {
              tr.push(dataList[i].high - dataList[i].low);
            } else {
              const prevClose = dataList[i - 1].close;
              const hl = dataList[i].high - dataList[i].low;
              const hc = Math.abs(dataList[i].high - prevClose);
              const lc = Math.abs(dataList[i].low - prevClose);
        ...
        styles: {
          lines: [
            { key: "mid", color: colors.mid, size: thickness, style: "solid" },
  ...
  useEffect(() => {
    ...
    const initKC = async () => {
      ...
      registerKC(klinecharts);
      const id = chart.createIndicator("MY_KC", true, { id: "candle_pane" });
  ...
  useEffect(() => {
    ...
    chart.overrideIndicator({
      ...
      styles: {
        lines: [
          { key: "mid", color: colors.mid, size: thickness },
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <input
                ...
                value={colors.upper}
                onChange={(e) => {
                  handleColorChange("upper", e.target.value);
                  handleColorChange("lower", e.target.value);
                }}
                onMouseDown={(e) => e.stopPropagation()}
                onClick={(e) => e.stopPropagation()}
                style={{
...
export default KeltnerChannelIndicator;
