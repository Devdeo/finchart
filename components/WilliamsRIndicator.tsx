...
interface WilliamsRIndicatorProps {
  chart: any;
  onRemove: () => void;
...
const WilliamsRIndicator: React.FC<WilliamsRIndicatorProps> = ({
  chart,
  onRemove,
  initialPeriod = 14,
  initialColor = "#FF5722"
  ...
  const registerWilliamsR = (klinecharts: any) => {
    try {
      klinecharts.registerIndicator({
        ...
        calc: (dataList: any[], indicator: any) => {
          ...
          for (let i = 0; i < dataList.length; i++) {
            ...
            const wr = high !== low ? ((high - close) / (high - low)) * -100 : 0;
            result.push({ wr });
          }
          return result;
        },
  ...
  useEffect(() => {
    ...
    const initWilliamsR = async () => {
      const klinecharts = await import("klinecharts");
      registerWilliamsR(klinecharts);
      const id = chart.createIndicator("MY_WR", false);
  ...
  useEffect(() => {
    ...
    chart.overrideIndicator({
      id: indicatorIdRef.current,
      styles: { lines: [{ key: "wr", color, size: thickness }] },
    });
