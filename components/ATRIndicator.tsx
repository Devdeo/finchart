...
const ATRIndicator: React.FC<ATRIndicatorProps> = ({
  chart,
  onRemove,
  initialPeriod = 14,
  initialColor = "#E91E63"
  ...
  const registerATR = (klinecharts: any) => {
    try {
      klinecharts.registerIndicator({
        ...
        calc: (dataList: any[], indicator: any) => {
          const p = Number(indicator.calcParams[0]) || 14;
          const result: any[] = [];

          for (let i = 0; i < dataList.length; i++) {
            const cur = dataList[i];
            const prev = dataList[i - 1] || cur;
            ...
            if (i === 0) {
              ...
              // simple average until enough periods
              const sum = result.reduce((acc, r) => acc + r.atr, 0) + tr;
              result.push({ atr: sum / (i + 1) });
            } else {
              // ATR = (prevATR * (p - 1) + TR) / p
              const prevATR = result[result.length - 1].atr;
              const atr = (prevATR * (p - 1) + tr) / p;
              result.push({ atr });
            }
          }
  ...
  useEffect(() => {
    ...
    const initATR = async () => {
      ...
      registerATR(klinecharts);
      const id = chart.createIndicator("MY_ATR", false);
  ...
  useEffect(() => {
    ...
    chart.overrideIndicator({
      id: indicatorIdRef.current,
      styles: { lines: [{ key: "atr", color, size: thickness }] },
    });
